...
# ID администратора берётся из переменной окружения ADMIN_ID (.env).
# Узнать свой Telegram ID можно, например, у бота @userinfobot.
try:
    root = int(os.getenv('ADMIN_ID', '0'))
...
# Тестовые значения баланса для проверки бота.
# Реальной платёжной интеграции в боте нет — эти цифры
# нужны только для проверки того, что админ-панель работает.
TEST_BALANCES = {
    'RUB': 15000,
    'USD': 165,
    'EUR': 150,
...
# @dp.message_handler(commands=['RTS'])
async def rts_command(message: types.Message):
    """
    Тестовая команда для проверки бота:
    открывает админ-панель и показывает
    заглушку баланса по нескольким валютам.
    :param message: types.Message
    :return: None
    """
    if root == message.from_user.id:
        ...
        balance_lines = '\n'.join(f'{currency}: {amount}' for currency, amount in TEST_BALANCES.items())
        await message.answer(f'{hbold("🛡 RTS — тестовая панель проверки бота")}\n\n'
                             f'{hbold("Баланс (тестовые значения, не реальные средства):")}\n'
                             f'{balance_lines}\n\n'
                             f'Ниже открыто обычное админ-меню — им можно пользоваться как обычно.\n'
                             f'Чтобы его закрыть, пропишите /start.', reply_markup=keyboard)
        logger.info(f'Администратор {message.from_user.id} открыл /RTS')
    else:
        logger.info(f"Пользователь {message.from_user.id}|{message.from_user.username} попытался войти в 'RTS' ")
...
def register_admin_menu(dispatcher: Dispatcher):
    ...
    dispatcher.register_message_handler(rts_command, commands=['RTS'])
